...
- **Workstream A — Library creation (build-time).**
  The reference games (~20) are generated manually by the development
  team using an LLM (in this conversation or a separate session),
  hand-edited through an editorial pass, then embedded via
  `text-embedding-3-small` and seeded into the two tables.
  Runs once (and again whenever the curated set is revised).
  The LLM is NOT called from this codebase — it's manual.
...
Two scripts in `apps/server/scripts/`, run in sequence by a developer
after curating the games. All scripts are Bun scripts (`bun run scripts/...`).
They use the existing shared db client and shared model id constants.

The reference games are NOT generated by a script in this repo. Instead:

1. **Generate the games manually** — using a strong LLM (Claude Opus, Sonnet, or equivalent) directly in this conversation or a separate LLM session. Copy the generated HTML into `apps/server/scripts/rag-curated/<id>.html`.
2. **Editorial pass** — hand-edit each game until it satisfies the SPEC §13 base contract (canvas, init/update/render/gameLoop, title screen, game over, key state map, procedural assets, self-contained, try/catch + postMessage) and plays in a browser.

Then run the embedding + seed scripts:
...
### Manual reference-game generation

The reference games are generated by the development team in this conversation
(or a separate LLM session), not via an automated script in this repo.
This ensures the highest-quality curated set — each game is hand-edited
and verified to play in a browser before being seeded.

- The development team uses a strong LLM (Opus, Sonnet, or equivalent) to generate ~20 games (~2–3 per genre).
- Each generated game is hand-edited until it satisfies the SPEC §13 base contract (canvas, init/update/render/gameLoop, title screen, game over, key state map, procedural assets, self-contained, try/catch + postMessage) and plays without errors.
- The curated HTML files are copied into `apps/server/scripts/rag-curated/`, then embedded and seeded via scripts.

Runtime continues to use Sonnet 4.6 (SPEC §3, §13) for all user-facing generation.
...
1. `bun run dev` starts the server and the shared db client logs a
   ...
   version string.
2. `apps/server/scripts/embed-rag-examples.ts` and
   `apps/server/scripts/seed-rag-examples.ts` exist and each runs
   end-to-end against real API keys. Embedding uses
   `text-embedding-3-small`. The curated games in
   `apps/server/scripts/rag-curated/` are committed to the repo.
   (The reference games are generated manually by the
   development team, not via a script in this repo.)
...
6. Server logs show, per generation request: an embedding call,
   ...
   (The reference games are generated manually, not via API.)
