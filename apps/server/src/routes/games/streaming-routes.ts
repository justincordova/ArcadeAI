// SSE streaming game routes: generate (POST /api/games), refine, and repair.
//
// Every handler here calls `reply.hijack()` to write raw SSE frames, which
// means Fastify's global error handler NO LONGER runs once hijacked. The
// shared invariants each handler upholds:
//   - credits are deducted BEFORE the stream and refunded on any failure;
//   - the per-user concurrency lock is acquired up front and released in a
//     `finally` on every exit path;
//   - `stopHeartbeat` / `logId` / `hijacked` are hoisted so the outer catch
//     can clean up regardless of where a throw lands;
//   - a pre-hijack throw is rethrown (Fastify answers it); a post-hijack
//     throw must be written as an SSE `error` frame by hand + `endSSE`.
...
import {
  ...
  markRepairFailed,
  markRepairSucceeded,
} from "../../services/usage/repair-log.js";
...
export function registerGameStreamingRoutes(app: FastifyInstance) {
  ...
  app.post("/api/games", { config: perUser10PerMin }, async (request, reply) => {
    ...
    try {
      ...
      // The Postgres transaction commits only after both awaited inserts finish.
