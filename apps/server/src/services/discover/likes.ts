...
export async function likeGame(gameId: string, userId: string): Promise<LikeResult | null> {
  return db.transaction(async (tx) => {
    ...
    // A concurrent like from the same user can collide with the
    ...
    return { liked: true, changed: true, likeCount: updated[0]?.likeCount ?? row.likeCount + 1 };
  });
}
...
export async function unlikeGame(gameId: string, userId: string): Promise<LikeResult | null> {
  return db.transaction(async (tx) => {
    ...
    const updated = await tx
      .update(games)
      .set({ likeCount: sql`GREATEST(${games.likeCount} - 1, 0)` })
      .where(eq(games.id, gameId))
      .returning({ likeCount: games.likeCount });
    ...
    return {
      liked: false,
      changed: true,
      likeCount: updated[0]?.likeCount ?? Math.max(0, row.likeCount - 1),
    };
  });
}
