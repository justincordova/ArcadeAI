...
| `WEB_ORIGIN` | The web app's origin (e.g. `http://localhost:5173`). CORS allowlist + Better Auth `trustedOrigins`. |
| `BETTER_AUTH_URL` | The **server's** public origin (e.g. `https://api.example.com`). OAuth callback URIs are built from this, so it must match what's registered with Google/GitHub. Required in production. |
| `VITE_API_BASE` | Build-time only, read by the SPA. Baked into the bundle by `vite build`; defaults to `http://localhost:3000`, so a production build that omits it ships a bundle that calls localhost. Set to `""` when the API is served same-origin (see `Dockerfile`). |
...
- **OAuth bounces back to the wrong host:** check `BETTER_AUTH_URL`. The
  provider callback URI is built as `${BETTER_AUTH_URL}/api/auth/callback/:provider`,
  so this is the value that must match what's registered with Google/GitHub.
  `WEB_ORIGIN` is a separate thing — it feeds the CORS allowlist and Better
  Auth's `trustedOrigins`, and is where you land *after* the callback
  completes.
